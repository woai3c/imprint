// Schema v1 compatibility only. New profiles use structured assertion validation.
...
const OUTLINED_BUTTON_ASSERTION =
  /\b(?:outlined?|bordered)(?:\s+pill(?:-shaped)?)?\s+(?:button|action|cta)s?\b|(?:描边|边框)(?:式)?(?:的)?(?:\s*(?:pill|胶囊(?:形|式)?))?\s*(?:按钮|行动|操作|CTA)|按钮[^。！？]{0,16}(?:描边|边框)/i
...
const SHAPE_DIRECTIVES = {
  pill: {
    avoid:
      /\b(?:avoid|do not|don't|never)\b[^.!?]{0,64}\bpill(?:-shaped)?\b|(?:避免|不要|不得|不能|切勿)[^。！？]{0,48}胶囊/i,
    preserve:
      /\b(?:keep|preserve|retain|reuse|remain|stay)\b[^.!?]{0,48}\bpill(?:-shaped)?\b|(?:保持|保留|沿用|复用|采用|使用)[^。！？]{0,36}胶囊/i,
  },
  sharp: {
    avoid:
      /\b(?:avoid|do not|don't|never)\b[^.!?]{0,64}\b(?:sharp|square)(?:-cornered)?\b|(?:避免|不要|不得|不能|切勿)[^。！？]{0,48}(?:直角|方角|sharp)/i,
    preserve:
      /\b(?:keep|preserve|retain|reuse|remain|stay)\b[^.!?]{0,48}\b(?:sharp|square)(?:-cornered)?\b|(?:保持|保留|沿用|复用|采用|使用)[^。！？]{0,36}(?:直角|方角|sharp)/i,
  },
} as const

function componentDirectiveScopes(text: string): Set<string> {
  const scopes = new Set<string>()
  if (/\b(?:primary|main)\s+(?:button|action|cta)\b|主按钮|主操作|主要操作|主要行动|主\s*CTA/i.test(text)) {
    scopes.add('primary-button')
  }
  if (/\bsecondary\s+(?:button|action)\b|次级按钮|次要按钮|二级按钮/i.test(text)) scopes.add('secondary-button')
  if (/\bicon(?:[- /]?buttons?)?\b|图标按钮|图标控件/i.test(text)) scopes.add('icon-button')
  if (/\btext(?:[- /]?buttons?)?\b|文本按钮|文字按钮/i.test(text)) scopes.add('text-button')
  if (/\binputs?\b|输入框|输入控件/i.test(text)) scopes.add('input')
  if (/\bcards?\b|卡片/i.test(text)) scopes.add('card')
  if (scopes.size === 0 && /\bbuttons?\b|按钮/i.test(text)) scopes.add('button')
  return scopes
}

function hasContradictoryShapeDirective(statement: string, implementation: string): boolean {
  const statementScopes = componentDirectiveScopes(statement)
  const implementationScopes = componentDirectiveScopes(implementation)
  if (statementScopes.size === 0 || implementationScopes.size === 0) return false
  const overlappingScope = [...statementScopes].some(
    (scope) => implementationScopes.has(scope) || (scope.endsWith('button') && implementationScopes.has('button')),
  )
  const reverseOverlap = [...implementationScopes].some(
    (scope) => statementScopes.has(scope) || (scope.endsWith('button') && statementScopes.has('button')),
  )
  if (!overlappingScope && !reverseOverlap) return false
  return Object.values(SHAPE_DIRECTIVES).some(
    (directive) =>
      (directive.avoid.test(statement) && directive.preserve.test(implementation)) ||
      (directive.preserve.test(statement) && directive.avoid.test(implementation)),
  )
}
...
function responsivePropertyLabel(property: string, language: DesignProfile['language']): string {
  const nodeProperty = property.match(/^node\.([^.]+)\.(.+)$/)
  const normalized = nodeProperty?.[2] || property
  const key = `intelligence.responsive.properties.${normalized}`
  const translated = coreT(language, key)
  const label = translated === key ? normalized.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`) : translated
  return nodeProperty ? `${label} (${nodeProperty[1]})` : label
}
...
export function checkProfileContradictions(
  ...
  for (const section of evidence.sections) {
    const components = evidence.components.filter((component) => component.sectionId === section.id)
    if (components.length > 0) componentsById.set(section.id, components)
  }
  const imageEvidenceIds = new Set(evidence.pages.flatMap((page) => page.images.map((image) => image.id)))
  ...
  const visit = (value: unknown, path: string): void => {
    ...
    if (isClaim) {
      ...
      if (hasContradictoryShapeDirective(claim.statement, claim.implementation)) {
        hardReject('internally-contradictory-shape-directive')
      }
      if (claim.tokenRefs) {
      ...
      const hasImageCitation = claim.evidence.some((reference) => imageEvidenceIds.has(reference.evidenceId))
      ...
      const directiveScopes = componentDirectiveScopes(text)
      const assertedComponentTypes = new Set<ComponentType>()
      if (directiveScopes.has('card')) assertedComponentTypes.add('card')
      if (directiveScopes.has('input')) assertedComponentTypes.add('input')
      if ([...directiveScopes].some((scope) => scope === 'button' || scope.endsWith('-button'))) {
        assertedComponentTypes.add('button')
      }
      if (!hasImageCitation && assertedComponentTypes.size > 0) {
        const referencedTypes = new Set(referencedComponents.map((component) => component.type))
        const missingTypes = [...assertedComponentTypes].filter((type) => !referencedTypes.has(type))
        if (missingTypes.length > 0) hardReject(`component-type-not-cited(${missingTypes.join('|')})`)
      }
