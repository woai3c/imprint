...
| npm CLI/MCP distribution                            | `SETUP REQUIRED` | `design-imprint@0.1.0` has an independent `cli-vX.Y.Z` workflow and packs only compiled CLI/core/MCP code, documentation and license. Its isolated tarball install passed both CLI aliases, doctor, real CLI extraction, official-client MCP discovery/extraction and filesystem cleanup on macOS. The registry name returned `E404`; no npm publication or tag workflow has run. | An npm owner performs the one-time bootstrap publication, configures Trusted Publishing for `cli-release.yml` with direct `npm publish` allowed, then verifies the tagged workflow and another platform. Recheck each package/release change. |
...
| Full E2E and release delivery                       | `PARTIAL`        | The local candidate separates Desktop `vX.Y.Z` releases, which retain full E2E/native builds, from CLI/MCP `cli-vX.Y.Z` releases, which gate unit, process and installed-package tests without building Desktop. Neither changed workflow has run remotely. Historical Desktop tag evidence remains scoped to its original revision.                                              | Release maintainer verifies the applicable channel workflow on the exact tag. Recheck after workflow, runner, signing, package or release-policy changes.                                                                                     |
