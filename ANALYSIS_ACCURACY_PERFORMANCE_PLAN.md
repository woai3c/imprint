...
状态：实施中；阶段 1 主链路已落地，等待在线配对 benchmark 复核质量与真实供应商耗时  
...
- API 路径默认输出上限为 4096 token，默认视觉输入最多 2 张；设计解读不会因 thinking 截断自动再次请求；
- AI 解读已记录 digest、图片处理、调用、验证、总耗时、token 和图片数量；程序提取的细分计时仍待阶段 0 后续补齐；
- 单元测试、类型检查、ESLint、CLI 构建和离线浏览器 benchmark 已通过；在线 AI 配对 benchmark 尚未执行。
...
| 视觉摘要/联系表替代多张长截图                | Imprint 性能约束                | 高             |             +1–3 秒 |           显著下降 | 必做           |
...
不再默认发送 4 张超长全页截图。程序生成最多 2 张视觉摘要：

1. **页面联系表**：入口 desktop/mobile 与代表子页面的缩略总览，用清晰边界标注短 page ID。
2. **显著区域板**：从 hero、workspace 主区、主要媒体和独有布局中选取 3–6 个裁剪区域，并标注短 section ID。
...
- 结构型页面可只发联系表；媒体风格明显时增加显著区域板；
- 原图和证据仍本地保存，AI 只接收摘要图。

预算：图片生成 ≤ 3 秒，压缩后单张建议 ≤ 250 KB，默认 1 张、最多 2 张。视觉 token 必须进入总输入预算。
...
- [ ] 实现统一 `AnalysisTiming`。
- [ ] 保存当前 14 个 benchmark fixture 的质量与耗时基线。
- [ ] 增加知乎类长页面、横向溢出、延迟活动弹层和不同子页面布局 fixture。
- [ ] 把 prompt 字符数、输入/输出 token、图片数和总 AI 时间加入 benchmark 结果。
...
- [ ] 持久化 digest/视觉摘要/AI 结果缓存。
- [ ] UI 展示“程序分析完成”和“AI 解读预计剩余阶段”，但不伪造精确剩余秒数。
- [ ] 增加显式“深度复核”，允许第二次 AI 调用并提示预计增加的时间和费用。
- [ ] 将完整逐组件规格和视觉 diff 作为开发/导出选项，不进入默认分析。
...
- 14 个现有 benchmark fixture 不退化，新增污染和 overflow fixture 全部通过。
...
| 图片减少导致视觉风格判断退化               | 使用联系表 + 显著区域板，按信息增益而非固定首页选图         |
