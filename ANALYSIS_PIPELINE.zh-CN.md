...
- AI 不修改令牌值。
- AI 当前只会提出**颜色令牌重命名**。
- AI 返回的设计摘要、设计意图和特征标签目前还没有被后续流程使用。
...
[`component-detect.ts`](./src/core/analyzer/component-detect.ts) 通过 DOM 选择器和可计算的外观特征识别常见组件：
...
例如，卡片不是靠 AI “看图”判断，而是根据圆角、阴影或边框、内边距等特征组合判断。为控制性能，候选数量和样本数量都有上限。

识别结果表示“页面中存在这类结构及其代表样式”，不是完整的组件源码还原。当前通常取第一个匹配样本，复杂页面中未必能代表所有变体。
...
> 颜色、字体、字号、字重、行高、间距、圆角、阴影、边框等数组在进入后续构建器前已经去重，导致后续通过数组再次统计频率时，很多值只剩一次。

`usageCount` 中的真实计数仍然存在，但颜色聚类和部分令牌构建逻辑没有统一使用它。这会让“按使用频率排序”的实际效果弱于设计意图。
...
```json
{
  ...
  "designSummary": "A restrained product interface with a strong blue action color.",
  "designIntent": "Prioritizes clarity and task completion.",
  "featureTags": ["clean", "productivity", "high-contrast"]
}
```
...
| 字段            | 设计目的                       | 当前是否实际使用 |
| --------------- | ------------------------------ | ---------------- |
| `colorNames`    | 把通用颜色名改成更语义化的名称 | 是               |
| `designSummary` | 总结视觉系统                   | 否               |
| `designIntent`  | 推测设计意图                   | 否               |
| `featureTags`   | 生成语义特征标签               | 否               |

当前代码虽然会解析后三个字段，但没有把它们写入最终分析结果、数据库或导出文档。最终设计文档仍使用确定性 `featureTags`。

### 5.5 AI 怎样修改最终结果

当前只应用 `colorNames` 映射，并且只修改颜色令牌的键名，保留原始值。
...
颜色值 `#2563eb` 没有改变。AI 当前不能增加一个新的颜色，也不能修改字号、间距、圆角或其他令牌。
...
```json
{
  ...
  "designSummary": "A clean interface with a blue action color.",
  "designIntent": "Emphasizes primary task completion.",
  "featureTags": ["clean", "focused"]
}
```
...
以下问题不表示分析不可用，而是说明当前“证据 → 令牌 → AI 语义”链路还有哪些可重构空间。

### 6.1 P0：数组提前去重导致频率信息损失

合并器在令牌构建前对多个字段去重。后续代码如果再根据数组统计出现次数，得到的通常只是“每个值一次”，无法代表它在真实页面中的使用频率。

建议：

- 原始采集层保留所有出现记录或可靠的聚合计数；
- 归一化后统一从计数表构建令牌；
- 不要同时维护一套数组频率和一套 `usageCount`，避免二者不一致；
- 去重应发生在最终展示或导出阶段，而不是证据层。

### 6.2 P0：AI 重命名缺少严格校验

当前会直接使用 AI 返回的新名称，尚未完整校验：
...
建议 AI 只提交“重命名提案”，由确定性校验器决定是否接受。任何不合法映射都应被逐项拒绝，而不是让整个分析失败。

### 6.3 P0：浅色和深色令牌可能失去名称对应关系

当前 AI 只接收和重命名浅色颜色令牌。深色令牌在之后单独构建，没有应用同一映射。

例如浅色模式从 `primary` 改成 `action-brand`，深色模式仍可能叫 `primary`。导出时就难以表达“同一个语义令牌在不同主题下的值”。
...
`designSummary`、`designIntent` 和 AI `featureTags` 已经消耗模型输出，但当前没有进入产品结果。

有两种合理选择：

1. 真正接入结果，并明确标注这些内容由 AI 推断；
2. 从提示词和返回协议中删除，降低 token、延迟和解析复杂度。

在产品用途确定之前，不建议继续让模型生成但随后丢弃。
...
组件识别通常使用第一个匹配样本，卡片启发式可能把嵌套容器也识别为卡片。
...
1. 统一频率数据源，避免合并去重损失次数。
2. 给 AI 重命名增加名称、来源和冲突校验。
3. 建立稳定 token ID。
4. 同一重命名映射同时应用到浅色和深色主题。
5. 决定接入或删除 AI 的 summary、intent、featureTags。
6. 明确界面和导出中哪些内容是确定性的，哪些是 AI 推断的。
...
项目当前没有单元测试框架，但仍可以使用 Node test runner 或在现有 E2E 中逐步覆盖以下场景。
