...
| npm CLI/MCP distribution                            | `PARTIAL`        | `design-imprint@0.1.1` is public from `cli-v0.1.1`. The Ubuntu workflow passed 955 unit tests, 29 CLI/MCP process tests and installed-package smoke tests; the tarball then passed the same macOS CLI/MCP extraction smoke path and matched the workflow artifact's registry SRI. Trusted Publishing authorizes `woai3c/imprint` workflow `cli-release.yml` without a GitHub npm token. | Bootstrap release used maintainer web authentication. Trusted Publishing has no GitHub Environment and allows direct CI `npm publish`; verify OIDC on the next immutable version and Windows installed-package behavior. Recheck each package/release change. |
...
- `PARTIAL`: after the bootstrap publication, npm Trusted Publishing was configured for `woai3c/imprint`, workflow
  `cli-release.yml`, no GitHub Environment, with direct `npm publish` and staged publishing allowed. A direct OIDC
  publication is not yet executed because `0.1.1` already existed when the successful workflow retry ran. Full Desktop
  E2E, Windows installed-package verification and a maintainer-selected MCP host also remain outside this evidence.
